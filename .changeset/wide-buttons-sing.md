...
Like the three `case`/`in` fixes this package already carries, it is applied by reopening the class at boot — it lives in `src/stree-perf-patch.ts`, so the artifact stays stock syntax_tree 6.3.0. Output is unchanged, and two things say so. `test/native-conformance.test.ts` asserts byte-identity against a native syntax_tree 6.3.0 on a source shaped to run through the comment walk. And a corpus comparison (`bun run ruby:bench --only corpus`, snapshotted before and after and `--compare`d) over 879 files of generated Ruby — 12 MB, the `.rb` half through syntax_tree and RuboCop, the `.rbi` half through syntax_tree alone — hashes every file the same, in 126 s against the 211 s the same run used to take. The worst file in that corpus, 568 KB carrying two accented characters, goes from 17.4 s to 2.7 s with the RuboCop pass off.
