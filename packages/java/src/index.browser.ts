 ...
 * it is the same google-java-format, and the same `createFormat` around it. The only
 * difference is where the wasm comes from, and `init` is here for the cases
 * where that needs saying explicitly.
 *
 * This is one of the smaller artifacts in the repo: 0.83MB over the wire,
 * 3.3MB of WasmGC to compile. It still wants a worker rather than the main
