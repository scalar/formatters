...
const checkRuntime = (): void => {
  ...
  if (version !== undefined && Number.parseInt(version, 10) < MINIMUM_NODE_MAJOR) {
    throw new Error(
      `@scalar/java-fmt needs Node ${MINIMUM_NODE_FOR_EXNREF} or newer (this is v${version}). ` +
  ...
  if (!supportsExnref()) {
    ...
    throw new Error(
      version === undefined
        ? '@scalar/java-fmt needs an engine that types a `catch_ref` label as the wasm spec does, ' +
