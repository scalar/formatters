...
import type { ArtifactSource, BootModule, RustFormatModule } from './types'
...
const finish = ({ wasi, workFiles, diagnostics }: Pending, instance: WebAssembly.Instance): RustFormatModule => {
 ...
 */
const instantiate = async (module: WebAssembly.Module): Promise<RustFormatModule> => {
 ...
 */
const instantiateSync = (module: WebAssembly.Module): RustFormatModule | undefined => {
  const pending = prepare()
  try {
...
export const createBootModule = (compileArtifact: ArtifactSource): BootModule => {
  let bootPromise: Promise<RustFormatModule> | undefined
   ...
   * an `await` can answer.
   */
  const peek = (): RustFormatModule | undefined => current
   ...
   */
  const recycle = (): RustFormatModule | undefined => {
    if (!compiled) {
