 ...
 * `rust_fmt.wasm.br` as an asset with a hashed name and rewrite the URL to match;
...
export const createArtifactLoader = (): {
  ...
  const readBytes = async (): Promise<Uint8Array> => {
    ...
    if (!response.ok) {
      throw new Error(`could not fetch the rustfmt artifact from ${String(url)} (HTTP ${response.status})`)
    }
