...
- 28460c0: The bundled RuboCop moves from 1.81.6 to 1.91.0. rubocop-ast stays at 1.50.0 and parser at 3.3.12.0, so both boot-time performance patches still apply to the versions they were written against. Among RuboCop's own dependencies, parallel moves to 2.2.0, regexp_parser to 2.13.0, unicode-display_width to 3.3.0 and unicode-emoji to 4.3.0. `@ruby/wasm-wasi` moves from 2.8.1 to 2.10.1, which matches the `js` gem 2.10.1 already built into the artifact.
...
- 597ac86: Pre-warm RuboCop's parsed default configuration into the wasm snapshot
